...
8. [macOS Distribution & Code Signing](#macos-distribution--code-signing)
9. [Developer Checklist (Before Publishing)](#developer-checklist-before-publishing)
10. [Git History Hygiene](#git-history-hygiene)
...
The PyInstaller spec (`Passport Filigrane.spec`) includes:
- `bundle_identifier='com.passportfiligrane.app'` -- Identifies the app to macOS
- `codesign_identity=None` -- **Must be set before release**
- `entitlements_file=None` -- **Must be created before release**

### Before Publishing a Release

1. **Obtain an Apple Developer certificate** (Developer ID Application)
2. **Set `codesign_identity`** in the `.spec` file to your signing identity
3. **Create an `entitlements.plist`** that restricts the app to necessary
   permissions only (file access, no network, no camera, etc.)
4. **Notarize the app** with `xcrun notarytool` so macOS Gatekeeper accepts it
...
- Users must bypass security warnings (right-click > Open), which trains them to
  ignore security prompts -- dangerous for an app handling identity documents
- There is no guarantee the binary hasn't been tampered with

### PyInstaller Version

Always build with PyInstaller >= 6.0.0 to avoid CVE-2025-59042 (arbitrary code
injection via `sys.path` manipulation).

---

## Developer Checklist (Before Publishing)

- [ ] Run `pip audit` to check for new dependency CVEs
- [ ] Ensure `pyinstaller --version` shows >= 6.0.0
- [ ] Set `codesign_identity` in `.spec` file
- [ ] Create and reference `entitlements.plist`
- [ ] Notarize the built `.app` with Apple
- [ ] Clean git history of personal data (see below)
- [ ] Verify `.gitignore` covers all sensitive patterns
- [ ] Test with a malformed PDF to verify input validation
- [ ] Test with a very large image to verify dimension limits
- [ ] Confirm error log writes to the correct platform directory
...
Git commits permanently record the author's name and email. Before making the
repository public, you should:

1. **Use GitHub's noreply email** for future commits:
   ```bash
   git config user.email "USERNAME@users.noreply.github.com"
   ```

2. **Rewrite existing history** to remove personal emails:
   ```bash
   ...
   ```
   New Name <new@email.com> <old-personal@email.com>
   ```

3. **Purge build artifacts** from history (reduces repo from ~178 MB to <5 MB):
   ```bash
   git filter-repo --path build/ --path dist/ --invert-paths
   ```

> **Warning**: `git filter-repo` rewrites all commit hashes. All collaborators
> must re-clone after this operation. This should be done once, before the
