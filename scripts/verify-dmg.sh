...
set -euo pipefail
cd "$(dirname "$0")/.."
dmg="${1:?Usage: verify-dmg.sh path/to/Privacy-Watermark.dmg [version]}"
...
app="$mount/Privacy Watermark.app"
...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$plist")" == com.lounim.privacywatermark ]] || { echo 'Unexpected bundle identity.' >&2; exit 1; }
[[ -s "$app/Contents/Resources/app_icon.icns" ]] || { echo 'Missing app icon.' >&2; exit 1; }
lipo "$app/Contents/MacOS/PrivacyWatermark" -verify_arch arm64 x86_64 || exit 1
...
for entry in "$mount"/* "$mount"/.[!.]*; do
    ...
    case "$(basename "$entry")" in
        'Privacy Watermark.app'|Applications|Install.txt|.background.png|.VolumeIcon.icns|.DS_Store|.fseventsd|.Trashes|.Spotlight-V100) ;;
